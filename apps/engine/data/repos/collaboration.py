...
from engine.core.types import CollaborationState, Dimension, MemberRef, OrgId, Signal
...
class PgCollaboration:
    ...
    async def observe(self, org_id: OrgId, member: MemberRef, signals: Sequence[Signal]) -> None:
        """Move this person's scores by signals, reading the row and writing it back."""
        if not signals:
            return
