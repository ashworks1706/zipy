...
def test_only_connected_and_enabled_tools_are_offered(cfg):
    ...
    assert registry.available(frozenset(), {}) == ["sandbox", "search"]
...
def test_the_sandbox_ships_on_and_needs_no_connected_account(cfg):
    """A fresh clone gets it. wiring.probe_sandbox turns it off when no runtime answers."""
    assert cfg.tools["sandbox"].enabled is True
    registry = Registry(cfg.tools)
    assert "sandbox" in registry.available(frozenset(), {})


def test_an_org_that_wants_no_sandbox_is_not_offered_it(cfg):
    registry = Registry(cfg.tools)
    off = {"sandbox": OrgToolConfig("sandbox", enabled=False, overrides={})}
    assert "sandbox" not in registry.available(frozenset(), off)
