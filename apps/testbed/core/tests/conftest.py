...
The evals build the engine's own gateway, so they need the same configuration and the same
request shape the engine's tests use. The engine's conftest does not reach here: a conftest
applies to its own directory and below.
"""
