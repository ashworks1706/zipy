...
data              SQLAlchemy tables, repositories, Fernet vault, Redis rate limiter and queue
...
the action type, validates the arguments, fetches and decrypts the org's credential, runs the tool,
validates its result, and writes an audit entry whatever happened. A failure (expired token, rate
limit, permission) goes back to the model as a failed tool result it can explain.
...
| Cache and queue | Redis 7: rate limits, job queue | `engine/data/cache.py` |
